...
void HistoryLogWidget::add(QString text) {
    ...
    //如果current不是第一个,修改图标
    if(current>=0){
        auto oldIcon=new QTableWidgetItem(QIcon("/home/tlss/repos/vrlab/MaskGenerator/src/resource/gray.ico"),"");
        this->setItem(current,0,oldIcon);
    }

    //创建新的一行
    auto newIcon =new QTableWidgetItem(QIcon("/home/tlss/repos/vrlab/MaskGenerator/src/resource/green.ico"),"");
...
void HistoryLogWidget::undo() {
    ...
    if(itemList.size()>1&&current>0){
        //current改成红色图标
        auto red =new QTableWidgetItem(QIcon("/home/tlss/repos/vrlab/MaskGenerator/src/resource/red.ico"),"");
        this->setItem(current,0,red);
        current--;
        //current改成绿色图标
        auto green =new QTableWidgetItem(QIcon("/home/tlss/repos/vrlab/MaskGenerator/src/resource/green.ico"),"");
...
void HistoryLogWidget::redo() {
    ...
    if(current<itemList.size()-1&&current>=0){
        //current改成灰色
        auto gray = new QTableWidgetItem(QIcon("/home/tlss/repos/vrlab/MaskGenerator/src/resource/gray.ico"),"");
        this->setItem(current,0,gray);
        current++;
        //current改成绿色
        auto green =new QTableWidgetItem(QIcon("/home/tlss/repos/vrlab/MaskGenerat/src/resource/green.ico"),"");
        this->setItem(current,0,green);
        this->clearSelection();
        this->selectRow(current);
    }
